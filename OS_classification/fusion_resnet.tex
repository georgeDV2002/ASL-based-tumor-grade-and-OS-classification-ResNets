...
% -------------------------------------------------------------
% Connection style toggle: 1=Straight, 2=Bezier, 3=TO, 4=Smooth
% -------------------------------------------------------------
\newcommand{\connectionstyle}{2}
...
\def\gapFilm{2.5}      % where film fuses
\def\gapStage{1.2}   % between residual stages
\def\gapStem{2.0}   % after stem
...
% --- Block 1 (downsample: s=2) ---
\pic[shift={(\gapFilm,0,0)}] at (add2b-east) { % or (add2-east) if that's your node
  RightBandedBox={
    name=stage3a,
    caption=,
...
\node[align=center] at ($(stage3a-south)!0.5!(stage3b_b-south) + (0,2.3)$)
  {\textbf{Stage 3: } 2$\times$ \textbf{ResBlock}};

% ======== 3D translucent wrapper ========
\pic[shift={(1.5,0,0)}] at (add2b-east) {
  Box={
    name=stage3wrap,
    caption=,
    fill=cyan!30!green!15,
...
% --- Block 1 (downsample: s=2) ---
\pic[shift={(\gapFilm,0,0)}] at (add3b-east) { % or (add3-east) if that's your node
  RightBandedBox={
    name=stage4a,
    caption=,
...
\node[align=center] at ($(stage4a-south)!0.5!(stage4b_b-south) + (0,1.8)$)
  {\textbf{Stage 4: } 2$\times$ \textbf{ResBlock}};

% ======== 3D translucent wrapper ========
\pic[shift={(1.7,0,0)}] at (add3b-east) {
  Box={
    name=stage4wrap,
    caption=,
    fill=cyan!30!green!15,
...
\pic[shift={(\denseHead,0,0)}] at (gap-east) {
  RightBandedBox={
    name=fc,
    caption=,
    xlabel={{"1",""}}, ylabel={\raisebox{-20pt}{1}}, zlabel=\qquad 1,
    fill=\FcColor,
...
% ===================================================
% Input
\pic[shift={($(stage3a-east)!0.5!(stage3b_b-west)+(0,0,16)$)}]
  at (stage3b_b-east)
  {Box={
    name=film_input,
    caption=,
    xlabel={{"1",""}}, ylabel={\raisebox{-20pt}{1}}, zlabel=3,
    height=1.5, width=1.5, depth=1.5,
    fill={rgb:green,5;black,1}
  }};
\node[anchor=west, font=\footnotesize] at ($(film_input-south)+(-1,-1.5,0)$){\shortstack{Input(3)\\Norm}};
...
\pic[shift={($(2,0,0)$)}]
  ...
  {Box={
    name=film_hidden_1,
    caption=,
    xlabel={{"1",""}}, ylabel={\raisebox{-20pt}{1}}, zlabel=128,
    height=1.5, width=1.5, depth=10,
    fill=\FcColor
  }};
\node[anchor=west, font=\footnotesize] at ($(film_hidden_1-south)+(-1,-1.5,0)$) {\shortstack{Dense(128)\\ReLU}};

% Input -> Hidden 1
\draw[connection]
  (film_input-east) -- (film_hidden_1-west);

% 2nd hidden layer
\pic[shift={($(2,0,0)$)}]
  at (film_hidden_1-east)
  {Box={
    name=film_hidden_2,
    caption=,
    xlabel={{"1",""}}, ylabel={\raisebox{-20pt}{1}}, zlabel=128,
...
\node[anchor=west, font=\footnotesize] at ($(film_hidden_2-south)+(-1,-1.5,0)$) {\shortstack{Dense(128)\\ReLU}};

% Hidden 1 -> Hidden 2
\draw[connection]
  (film_hidden_1-east) -- (film_hidden_2-west);

% Beta head
\pic[shift={($(2,2,0)$)}]
  at (film_hidden_2-east)
  {Box={
    name=film_beta,
    caption=,
    xlabel={{"1",""}}, ylabel={\raisebox{-20pt}{256}}, zlabel=1,
    height=1.5, width=20, depth=1.5,
    fill=lime!60!black
  }};
\node[anchor=west] at ($(film_beta-east)+(0.5,0,0)$) {$\mathbf{\beta}$};

% Hidden 2 -> Beta
\draw[connection]
  (film_hidden_2-north)
  -- ($(film_beta-west -| film_hidden_2-north)$)
  -- (film_beta-west);
...
\node[anchor=west] at ($(film_gamma-east)+(0.5,0,0)$) {$\mathbf{\gamma}$};

% Hidden 2 -> Gamma
\draw[connection]
  (film_hidden_2-north)
  -- ($(film_gamma-west -| film_hidden_2-north)$)
  -- (film_gamma-west);

% Fusion connection paths
\path ($(film_beta-farwest)!0.5!(film_beta-fareast)$) coordinate (beta_back_mid);
\path ($(stage3a-west)+(-1.5,0,0)$) coordinate (s3a_target);
%\path ($(film_gamma-farwest)!0.5!(film_gamma-fareast)$) coordinate (gamma_back_mid);
\path ($(film_gamma-northwest)!0.5!(film_gamma-northeast)$) coordinate (gamma_back_mid);
\path ($(stage4b_b-east)+(1.5,0,0)$) coordinate (s4bb_target);

\ifcase\connectionstyle
  % ============================================
  % Case 0 (unused)
  % ============================================
...
\or
  ...
  \begin{pgfonlayer}{background}
    % Beta
    \draw[connection] (beta_back_mid) -- (s3a_target);
    \draw[connection] (beta_back_mid) -- (s4bb_target);
    % Gamma
    \draw[connection] (gamma_back_mid) -- (s3a_target);
    \draw[connection] (gamma_back_mid) -- (s4bb_target);
  \end{pgfonlayer}
...
\or
  ...
  % ===== Curves with controlled tangents (start: -z, mid: +x, end: +x) =====
  % knobs (tune to taste)
  \def\kzstart{-1.0}
  \def\kx{4.5}
  \def\kzend{2.0}
  ...
  % convenience points
  \path (beta_back_mid)  coordinate (Sbeta);
  \path (s3a_target)     coordinate (E3);
  \path (s4bb_target)    coordinate (E4);
  \path (gamma_back_mid) coordinate (Sgam);

  % choose mid waypoints (anywhere between start & end is fine)
  \path ($(Sbeta)!0.60!(E3)+(0.9,0.2,0.9)$) coordinate (Mbeta3);
  \path ($(Sbeta)!0.55!(E4)+(0,0,0)$) coordinate (Mbeta4);
  \path ($(Sgam)!0.50!(E3)+(0,0.6,0.9)$)  coordinate (Mgam3);
  \path ($(Sgam)!0.55!(E4)+(0,0,0)$)  coordinate (Mgam4);

  \begin{pgfonlayer}{background}
    % ---- Beta -> Stage 3
      \draw[connection]
        (Sbeta)
          .. controls ($(Sbeta)+(0,0,\kzstart)$) and ($(Mbeta3)+(\kx,0,0)$) ..
        (Mbeta3)
          .. controls ($(Mbeta3)+(-\kx,0,0)$) and ($(E3)+(0,0,\kzend)$) ..
        (E3);

    % ---- Beta -> Stage 4
    \draw[connection]
      (Sbeta)
        .. controls ($(Sbeta)+(0,0,-\kzstart)$) and ($(Mbeta4)+(-\kx,0,0)$) ..
      (Mbeta4)
        .. controls ($(Mbeta4)+(\kx,0,0)$) and ($(E4)+(0,0,\kzend)$) ..
      (E4);
  
    % ---- Gamma -> Stage 3
    \draw[connection]
      (Sgam)
        .. controls ($(Sgam)+(0,0,\kzstart)$) and ($(Mgam3)+(\kx,0,0)$) ..
      (Mgam3)
        .. controls ($(Mgam3)+(-\kx,0,0)$) and ($(E3)+(0,0,\kzend)$) ..
      (E3);
  
    % ---- Gamma -> Stage 4
    \draw[connection]
      (Sgam)
        .. controls ($(Sgam)+(0,0,-\kzstart)$) and ($(Mgam4)+(-\kx,0,0)$) ..
      (Mgam4)
        .. controls ($(Mgam4)+(\kx,0,0)$) and ($(E4)+(0,0,\kzend)$) ..
      (E4);
  \end{pgfonlayer}
...
\or
  % ============================================
  % 3: to[out=..,in=..,looseness=..]
  % ============================================
  \begin{pgfonlayer}{background}
    % Beta
    \draw[connection] (beta_back_mid) to[out=180,in=120,looseness=1.1] (s3a_target);
    \draw[connection] (beta_back_mid) to[out=0,in=60,looseness=1.1] (s4bb_target);
    % Gamma
    \draw[connection] (gamma_back_mid) to[out=180,in=240,looseness=1.1] (s3a_target);
    \draw[connection] (gamma_back_mid) to[out=0,in=300,looseness=1.1] (s4bb_target);
  \end{pgfonlayer}
...
\or
  ...
  \begin{pgfonlayer}{background}
    ...
    \path let \p1 = (gamma_back_mid), \p2 = (s3a_target) in
      coordinate (g13_mid) at ($(\p1)!.5!(\p2)$);
    \path let \p1 = (gamma_back_mid), \p2 = (s4bb_target) in
      coordinate (g14_mid) at ($(\p1)!.5!(\p2)$);
  
    % Beta
    \draw[connection] plot[smooth,tension=0.7]
      coordinates {(beta_back_mid) ($(beta_back_mid)+(-1,0.4,0)$) (s3a_target)};
    \draw[connection] plot[smooth,tension=0.7]
      coordinates {(beta_back_mid) ($(beta_back_mid)+(1,0.4,0)$) (s4bb_target)};
  
    % Gamma (add midpoints with tiny vertical offset to guide the curve)
    \draw[connection] plot[smooth,tension=0.7]
      coordinates {(gamma_back_mid) ($(g13_mid)+(0,-0.3,0)$) (s3a_target)};
    \draw[connection] plot[smooth,tension=0.7]
      coordinates {(gamma_back_mid) ($(g14_mid)+(0,-0.3,0)$) (s4bb_target)};
  \end{pgfonlayer}
