...
OS_COL = "OS"                  # time in DAYS
EV_COL = "1-dead 0-alive"      # 1=dead, 0=alive/censored
...
def background_ranges_with_overlay(
    ...
    for i in scans_idx:
        ...
        if save_masks_nifti:
            ...
            # save mask with same affine/header
...
def augment_dataset_bin(X, y, *, seed=123, per_sample=4):
    """
    Duplicate binary label y for each augmentation.
    X: (N, X, Y, Z, C)
    y: (N,) with {0,1} or NaN for unlabeled
    Returns (X_out, y_out)
    """
    rng = np.random.default_rng(seed)
    X = np.asarray(X)
    ...
    def _rand_flip(x, rng):
        ...
        x2 = x.copy()
        for ax in flip_axes: x2 = np.flip(x2, axis=ax)
        return x2
    ...
    def _elastic_deform(x, rng, alpha=2.0, sigma=8.0):
        Xd,Yd,Zd,C = x.shape
        dx = gaussian_filter(rng.normal(0,1,size=(Xd,Yd,Zd)), sigma=sigma)*alpha
        dy = gaussian_filter(rng.normal(0,1,size=(Xd,Yd,Zd)), sigma=sigma)*alpha
        dz = gaussian_filter(rng.normal(0,1,size=(Xd,Yd,Zd)), sigma=sigma)*alpha
        gx,gy,gz = np.meshgrid(np.arange(Xd), np.arange(Yd), np.arange(Zd), indexing='ij')
        coords = np.array([gx+dx, gy+dy, gz+dz])
        out = np.empty_like(x)
        for c in range(C):
            out[..., c] = map_coordinates(x[..., c], coords, order=1, mode='nearest')
        return out
    ...
    AUGS = [_rand_flip, _small_rotation, _elastic_deform, _rician_noise]
    ...
    for i in range(X.shape[0]):
        ...
        for j in range(per_sample):
            fn = AUGS[rng.integers(0, len(AUGS))]
            ss = np.random.SeedSequence([seed, i, j])
            sub_rng = np.random.default_rng(ss.generate_state(1, dtype=np.uint32)[0])
            X_out.append(fn(X[i], sub_rng)[None, ...])
            y_out.append(np.array([yi], dtype=float))
...
def save_split_bin(out_dir,
                   X_train, y_train, ids_train, paths_train,
    ...
    def _stats(y):
        ...
        mask = np.isfinite(y)
        if not np.any(mask):
            return {"labeled_count": 0, "positives": 0, "prevalence": np.nan}
        yy = y[mask]
        return {
    ...
    meta = {
        ...
        "train_labels": _stats(y_train),
        "test_labels":  _stats(y_test),
        "task": "binary_classification (1=dead, 0=alive)",
        "notes": notes,
    }
    (out_dir / "metadata.json").write_text(json.dumps(meta, indent=2))
    print(f"[OK] Saved binary split to {out_dir.resolve()}")
...
def load_paths_time_event(asl_dir=ASL_DIR, excel_path=EXCEL, max_scans=None):
    paths, ids_4d, ids_3d = [], [], []
    for p in sorted(Path(asl_dir).glob("*_ASL.nii.gz")):
        ...
        if not m:
            continue
        full4 = m.group(1)      # e.g. UCSF-PDGM-0123
        digits4 = m.group(2)    # e.g. 0123
        if digits4 in EXCLUDE_IDS:
            continue
        ids_4d.append(full4)
        ids_3d.append(f"UCSF-PDGM-{digits4[-3:]}")  # 3-digit form
        paths.append(str(p))
    ...
    # --- Required columns ---
    missing_cols = [c for c in [OS_COL, EV_COL, "Age at MRI", "Sex", "EOR"] if c not in df.columns]
    if missing_cols:
        raise ValueError(f"Excel missing required columns: {missing_cols}")

    # --- Robust ID normalization to 3-digit form used above ---
    def _norm_excel_id(x):
    ...
    # Try to find an ID column by your earlier logic
    id_col = None
    for c in ["ID", "Subject ID", "SubjectID", "Subject", "Patient", "Case"]:
        if c in df.columns:
            id_col = c
            break
    if id_col is None:
    ...
    # --- Build maps for survival columns (keep NaNs) ---
    time_map  = dict(zip(df["_norm_id3"], pd.to_numeric(df[OS_COL], errors="coerce")))
    event_map = dict(zip(df["_norm_id3"], pd.to_numeric(df[EV_COL], errors="coerce")))

    # --- Tabular inputs (keep floats so NaNs survive) ---
    age_series = pd.to_numeric(df["Age at MRI"], errors="coerce")  # float
    ...
    sex_series = df["Sex"].map(norm_sex)
    eor_series = df["EOR"].map(norm_eor)

    age_map = dict(zip(df["_norm_id3"], age_series))
    sex_map = dict(zip(df["_norm_id3"], sex_series))
    eor_map = dict(zip(df["_norm_id3"], eor_series))

    # --- Assemble aligned arrays for all found scans (ids_3d order) ---
    time_days = [time_map.get(i3, np.nan) for i3 in ids_3d]
    event     = [event_map.get(i3, np.nan) for i3 in ids_3d]
    age_all   = [age_map.get(i3, np.nan)  for i3 in ids_3d]
    sex_all   = [sex_map.get(i3, np.nan)  for i3 in ids_3d]
    eor_all   = [eor_map.get(i3, np.nan)  for i3 in ids_3d]

    return (np.array(time_days, dtype=float),
            np.array(event, dtype=float),
            np.array(ids_4d), np.array(paths),
            np.array(age_all, dtype=float),
            np.array(sex_all, dtype=float),
            np.array(eor_all, dtype=float))
...
if __name__ == "__main__":
    # --- 1) Load labels/ids/paths ---
    time_days, event, ids, paths, age_all, sex_all, eor_all = load_paths_time_event(max_scans=None)
    tab_all = np.stack([age_all, sex_all, eor_all], axis=1).astype(np.float32)  # (N, 3)

    # --- 2) Use ONLY binary labels; labeled rows = finite event ---
    is_labeled = np.isfinite(event)
    y_lab      = event[is_labeled].astype(int)
    ...
    # Stratify by binary label for test split
    desired_test = 0.20
    stratify_vec = y_lab if len(np.unique(y_lab)) > 1 else None

    ids_tr, ids_te, y_tr, y_te, p_tr, p_te = train_test_split(
        ids_lab, y_lab, paths_lab,
        test_size=desired_test, random_state=42, shuffle=True,
        stratify=(stratify_vec if stratify_vec is not None else None)
    )

    # Assemble final TRAIN/TEST lists
    ids_train   = np.concatenate([ids_tr,   ids_unlab])
    paths_train = np.concatenate([p_tr,     paths_unlab])
    y_train     = np.concatenate([y_tr,     np.full(len(ids_unlab), np.nan)])

    ids_test, paths_test, y_test = ids_te, p_te, y_te

    # --- 3) Prepare streaming build config ---
    CHANNELS = ("raw","zg","zl","log"); C = len(CHANNELS)
    SIDE_MM  = 70.0
    TOP_PCT  = 0.01
    ...
    y_all     = np.array(event)  # binary labels (may include NaNs)

    idx_train = np.array([id_to_idx[sid] for sid in ids_train])
    idx_test  = np.array([id_to_idx[sid] for sid in ids_test])

    tab_train_base = tab_all[idx_train]   # before augmentation (N_train_base, 3)
    tab_test       = tab_all[idx_test]    # test has no augmentation
    ...
    with tqdm(total=len(idx_test), desc="TEST  ", unit="scan") as pbar:
        for k, i in enumerate(idx_test):
            ...
            if xc.shape != tgt_shape:
                xc = pad_to_shape(xc, tgt_shape)
            X_test[k] = xc
            pbar.update(1)

    # --- 5) Build TRAIN (streamed + augment per labeled sample) ---
    def per_sample_augs_bin(y):
        return 0 if (not np.isfinite(y)) else 4

    n_total = len(idx_train) + sum(per_sample_augs_bin(y_all[i]) for i in idx_train)
    X_train   = np.zeros((n_total, *tgt_shape), dtype=np.float32)
    y_out     = np.empty((n_total,), dtype=float)
    ids_out   = np.empty((n_total,), dtype=object)
    paths_out = np.empty((n_total,), dtype=object)
    tab_train = np.zeros((n_total, 3), dtype=np.float32)

    w = 0
    with tqdm(total=n_total, desc="TRAIN ", unit="sample") as pbar:
        for i in idx_train:
            p   = paths_all[i]
            y_i = y_all[i]   # may be NaN (unlabeled)
            ...
            Xa, ya = augment_dataset_bin(x[None, ...], np.array([y_i]), seed=42+i, per_sample=4)
            nn = Xa.shape[0]

            X_train[w:w+nn]   = Xa
            y_out[w:w+nn]     = ya
            ids_out[w:w+nn]   = ids[i]
            paths_out[w:w+nn] = paths[i]

            tab_vec = tab_all[i]
            tab_train[w:w+nn, :] = tab_vec[None, :].repeat(nn, axis=0)

            w += nn
            pbar.update(nn)

    # --- 6) Save (binary) ---
    print(f"Train: X={X_train.shape}, labeled={np.sum(np.isfinite(y_out))}", flush=True)
    print(f"Test : X={X_test.shape},  labeled={np.sum(np.isfinite(y_test))}", flush=True)

    out_dir = Path("final_data_aug_bin")
    save_split_bin(
        out_dir,
        X_train, y_out, ids_out, paths_out,
        X_test,  y_test, ids_test, paths_test,
        notes=f"Streamed build (side={SIDE_MM}mm, channels={CHANNELS}, seed=42). Binary target from '{EV_COL}'."
    )

    np.save(out_dir / "tab_train.npy", tab_train)    # (n_total, 3) replicated per aug
    np.save(out_dir / "tab_test.npy",  tab_test)     # (N_test, 3)

    print("Tab shapes:", tab_train.shape, tab_test.shape, flush=True)
